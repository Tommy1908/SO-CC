...
static int recv_body(int fd, long long len)
{
	...
	int chunk_size = 64; // Otro diferente al del envio xque se puede
	char chunk[chunk_size];
	int number_bytes;
	long long remaining = len;

	while(remaining > 0){
		number_bytes = recv(fd,chunk, min(sizeof(chunk),remaining), 0);
		fwrite(chunk, 1, number_bytes, stdout);
		remaining -= number_bytes;
	}
