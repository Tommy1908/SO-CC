...
static int send_file(int fd, FILE *file)
{
	...
	int number_bytes;
	while(1){ 
		number_bytes = fread(chunk, 1, sizeof(chunk), file);
		if (number_bytes == 0){
			break;
		}
		if(send(fd, &chunk, number_bytes, 0) == -1){
...
int main(int argc, char **argv)
{
	...
	for (;;) {
		...
		// Handlear que pasa si fuimos interrumpidos
		if(client_fd<0){
			if(errno==EINTR){
		...
		pid_t son = fork();
		if(son == 0){
			close(listen_fd); // el hijo no tiene que saber de escuchar ninguno nuevo
			//printf("[%d] Voy a encargarme de un cliente\n", getpid());
