...
static void run_cmd(char **argv, int pipes[][2], int count, int i)
{
	...
	// borramos todos porque dup2 ya duplico lo que necesitabamos
...
static void run_pipeline(const pipeline_t *p)
{
	 ...
	 *    con n == 1. */
	 ...
	 // 1 solo comando necesita 0 pipes, y 1 pipe por cada uno nuevo.
	int (*pipes)[2] = NULL; // puntero null donde vamos a crear nuestra lista de pipes luego
	if(p->count > 1){
