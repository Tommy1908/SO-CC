...
```c
void nieto(int i, int pipe_nieto[2]){
    close(pipe)
    int r = calculo(i);
    pipe_nieto(pipe_nieto[WRITE], &r, sizeof(r));
    exit();
}
...
void ejecutarHijo (int i, int pipes[][2]) {
    ...
    int ans;
    while(!read(pipe_nieto[READ],&ans,sizeof(ans))){
        write(pipe_nieto[READ],&ans,sizeof(ans));
    }
}
```

ESTO ESTA MAL/INCOMPLETO, salteo por ahora pero habria q terminarlo